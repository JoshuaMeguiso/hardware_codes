...
{
    unsigned char tmp = 0;
    tmp = MFRC522_Rd_2(addr) ;
    MFRC522_Wr_2(addr, tmp&~mask);
}
...
{
    unsigned char stt;
    stt = MFRC522_Rd_2(0x14);
    MFRC522_Set_Bit_2(0x14, 0x03);
}

void MFRC522_AntennaOff_2(void)
{
    MFRC522_Clear_Bit_2(0x14, 0x03);
}
...
void MFRC522_Init_2(void)
{
    ...
    MFRC522_Reset_2();
    MFRC522_Wr_2(0x2A, 0x8D);
    MFRC522_Wr_2(0x2B, 0x3E);
    MFRC522_Wr_2(0x2D, 30);
    MFRC522_Wr_2(0x2C, 0);
    MFRC522_Wr_2(0x15, 0x40);
    MFRC522_Wr_2(0x11, 0x3D);
    MFRC522_AntennaOff_2();
    MFRC522_AntennaOn_2();
}
...
char MFRC522_ToCard_2(char command, char *sendData, char sendLen, char *backData, unsigned *backLen)
{
    ...
    MFRC522_Wr_2(0x02, irqEn | 0x80);
    MFRC522_Clear_Bit_2(0x04, 0x80);
    MFRC522_Set_Bit_2(0x0A, 0x80);
    MFRC522_Wr_2(0x01, 0x00);

    for(i=0; i<sendLen; i++)
    {
        MFRC522_Wr_2(0x09, sendData[i]);
    }
    MFRC522_Wr_2(0x01, command);
    if(command == 0x0C)
    {
        MFRC522_Set_Bit_2(0x0D, 0x80);
    }
    i = 0xFFFF;
    do
    {
    ...
    MFRC522_Clear_Bit_2(0x0D, 0x80);
    if(i != 0)
    {
        if(!(MFRC522_Rd_2(0x06) & 0x1B))
        {
            _status = 0;
            if(n & irqEn & 0x01)
            {
            ...
            {
                n = MFRC522_Rd_2(0x0A);
                lastBits = MFRC522_Rd_2(0x0C) & 0x07;
                if(lastBits)
                {
...
char MFRC522_Request_2(char reqMode, char *TagType)
{
    char _status;
    unsigned backBits;
    MFRC522_Wr_2(0x0D, 0x07);
    TagType[0] = reqMode;
    _status = MFRC522_ToCard_2(0x0C, TagType, 1, TagType, &backBits);
...
{
    char i, n;
    MFRC522_Clear_Bit_2(0x05, 0x04);
    MFRC522_Set_Bit_2(0x0A, 0x80);
    for(i=0; i<length; i++)
    {
        MFRC522_Wr_2(0x09, *dataIn++);
    }
    MFRC522_Wr_2(0x01, 0x03);
    i = 0xFF;
    do
    {
        n = MFRC522_Rd_2( 0x05 );
    ...
    while(i && !(n & 0x04));
    dataOut[0] = MFRC522_Rd_2(0x22);
    dataOut[1] = MFRC522_Rd_2(0x21);
}
...
char MFRC522_SelectTag_2(char *serNum)
{
    ...
    MFRC522_CRC_2(buffer, 7, &buffer[7]);
    _status = MFRC522_ToCard_2(0x0C, buffer, 9, buffer, &recvBits);
...
void MFRC522_Halt_2(void)
{
    ...
    buff[0] = 0x50;
    buff[1] = 0;
    MFRC522_CRC_2(buff, 2, &buff[2]);
    MFRC522_Clear_Bit_2(0x08, 0x80);
    MFRC522_ToCard_2(0x0C, buff, 4, buff, &unLen);
    MFRC522_Clear_Bit_2(0x08, 0x08);
}
...
char MFRC522_AntiColl_2(char *serNum)
{
    ...
    char serNumCheck = 0;
    unsigned unLen;
    MFRC522_Wr_2(0x0D, 0x00);
    serNum[0] = 0x93;
    serNum[1] = 0x20;
    MFRC522_Clear_Bit_2(0x08, 0x08);
    _status = MFRC522_ToCard_2(0x0C, serNum, 2, serNum, &unLen);
    if(_status == 0)
    {
        for(i=0; i<5; i++)
        {
...
char MFRC522_IsCard_2(char *TagType)
{
    if(MFRC522_Request_2(0x26, TagType) == 0)
        return 1;
    else
        return 0;
}
...
{
    char _status;
    _status = MFRC522_AntiColl_2(str);
    str[5] = 0;
    if(_status == 0)
        return 1;
    else
