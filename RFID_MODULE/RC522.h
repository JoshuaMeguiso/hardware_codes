/*/*
 * Prohibited commercial use. Do not use this code for commercial use.
 *
 * Warning: All Right reserveds to respective autors,
 * use this code only reference.
 *
 * The authors are not liable for damages resulting in any way.
 */
/*
 * File name: RFID
 * Creator: Dr.Leong   ( WWW.B2CQSHOP.COM ) (for ARDUINO)
 * Creation date: 2011.09.19
 * Modified by: R
 * Modified date: 2013.09.12
 * Modified: Adapted from Arduino to MikroC and added send serial format RFID-RDM630 by Bluetooth w/ HC-06 slave module
 * Functional Description: Mifare1 Anti-collision find cards - Select card reader interface
 * Modified by: Ahmad Afana
 * Modified date: 2018.12.19
 * Modification: interfacing the rfid (rc522) module with pic18f4550 using soft spi
 * and providing an example that reads the cards serial number and print it on a lcd
 */

/*

 *
 * RFID-RC522 MODULE 13.56MHz READER AND WRITER (MFRC522 CHIP) - SPI
 *   RFID-RC522(3.3V) = 3.3V
 *   RFID-RC522(RST) = ANODE DIODE 1N4148 TO PIC
 *   RFID-RC522(GND) = GND
 *   RFID-RC522(SDA) = SlaveSelect (SS) TO PIC


 * PIC18F4550 CONECTION:
 * (USE AN 11.0592MHz CRYSTAL FOR BEST RESULTS W/ UART - SERIAL: 9600,8,1,N)
 *   PIN     PORT  FUNCTION    CONNECTION
 *   4       A5    MCLR        OPEN (DISABLE) - ICSP PROG VPP (AT 12V!)
 *   16      A7    OSC1        CRYSTAL 11.0592MHz (PIN1)
 *   15      A6    OSC2        CRYSTAL 11.0592MHz (PIN2)
 *   11      B5    NRSTPD      > KATODE DIODE 1N4148 TO RFID-RC522(RST) (NRSTPD SEE DATASHEET MFRC522)
 *   18      A0    SPI SS      RFID-RC522(SDA) - SlaveSelect (SS)
 *   2       A3    SPI SDI     RFID-RC522(MOSI)
 *   3       A2    SPI SDO     RFID-RC522(MISO)
 *   17      A1    SPI SCK     1RFID-RC522(SCK)
 *   5       GND   GND
 *   14      V+    5V
 *
 */
...
#define  chipSelectPin RA0_bit       // chipSelect (CS) = SlaveSelect (SS)
#define  NRSTPD RB2_bit   // RESET AND POWER DOWN - SEE DATASHEET MFRC522
...
uchar old_led = 0;
const ushort inputStringLen = 1;
uchar  inputString[inputStringLen] = "";         // a string to hold incoming data
boolean stringComplete;  // whether the string is complete
ushort bytevar3, bytevar4, Output1, receiv_cnt = 0;
ushort receive[8];
...
/*
 * Function Name: TimeOut1
 * Function Description: TimeOut1 to reset bluetooth module ("key" pin) +/- 10s
 * Input Parameters: none
 * Return value: none
 */
...
/*
 * RECEIVING:
 * A remote device sends by bluetooth a frame with string "#ON1#" for change the output "Output1" to state HIGH.
 * A remote device sends by bluetooth a frame with string "#OFF1#" for change the output "Output1" to state LOW.
 * ANY DATA RECEIVED CLEARS THE TIMEOUT TO RESET BLUETOOTH MODULE
 */
/*
 * Function Name: INTERRUPT
 * Function Description: SerialEvent occurs
 * Input Parameters: none
 * Return value: none
 */
void interrupt(void)
{
  ...
  if (PIR1.RCIF) // USART Receive Interrupt Flag bit
  {
    PIR1.RCIF = 0;
    bytevar3=0; // CLEAR THE TIMEOUT TO RESET BLUETOOTH MODULE
    bytevar4=0; // CLEAR THE TIMEOUT TO RESET BLUETOOTH MODULE
    RCREG_temp = RCREG;
    ...
    if (receive[2] == 70) // 70 = "F"  - (#OFF1# ???)
    {
       receive[2] = 0;
       Output1 = 0;
       stringComplete = 1;
    }
    if (receive[2] == 78)  // 78 = "N" - (#ON1# ???)
    {
       receive[2] = 0;
       Output1 = 1;
       stringComplete = 1;
